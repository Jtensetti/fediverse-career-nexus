...
import { getSectionVisibility, canViewSection, type SectionVisibilityMap, type SectionVisibility, type ProfileSection } from "@/services/profile/sectionVisibilityService";
import SectionVisibilityToggle from "@/components/settings/SectionVisibilityToggle";
  ...
  return (
    <DashboardLayout showHeader={false}>
      ...
      <SEOHead
        ...
        description={profile.headline || profile.bio || t("profile.seoDescription", { name: profile.displayName })}
        ...
        <div className="container max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          {!viewingOwnProfile && !isAuthenticated && (
            <Button size="sm" asChild>
              <Link to="/auth/signup">
                <UserPlus className="h-4 w-4 mr-1" /> {t("profile.signUpToConnect")}
              </Link>
            </Button>
          )}
        </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {viewingOwnProfile ? (
                ...
                <>
                  <Button asChild>
                    <Link to="/auth/signup">
                      <UserPlus className="h-4 w-4 mr-2" /> {t("profile.signUpToConnect")}
                    </Link>
                  </Button>
                  <ShareButton
                    url={`${window.location.origin}/profile/${profile.username}`}
