...
export default function LinkedInImportModal({ open, onOpenChange, onImportComplete }: LinkedInImportModalProps) {
  ...
  const [importOptions, setImportOptions] = useState<ImportOptions>({
    includeProfile: true, includeExperiences: true, includeEducation: true, includeSkills: true, includeArticles: true,
  });
  ...
  const handleClose = () => {
    setCurrentStep('instructions'); setImportData(null); setImportResult(null); setError(null); setIsProcessing(false); onOpenChange(false);
  };

  const handleComplete = () => { handleClose(); onImportComplete?.(); };
