...
import { documents, LegalKind, legalOperator } from "@/components/legal/legalContent";
...
export default function LegalDocument({ kind, embedded = false }: { kind: LegalKind; embedded?: boolean }) {
  ...
  const content = <article className="prose dark:prose-invert max-w-none">
    ...
    <section><h2>{sv ? "Kontakt" : "Contact"}</h2><p>{legalOperator}<br /><a href={`mailto:${legalContact}`}>{legalContact}</a></p></section>
  </article>;
  ...
  return <div className="container max-w-3xl px-4 py-10 sm:py-16">
    ...
    <header className="my-8 space-y-3"><h1 className="text-3xl font-bold">{doc.title}</h1><p className="text-lg text-muted-foreground">{doc.intro}</p><p className="text-sm text-muted-foreground">{sv ? "Senast reviderad" : "Last revised"}: 2026-09-22</p></header>
