...
export default function InformationPage({ title, intro, sections }: Props) {
  ...
  return <div className="container max-w-3xl px-4 py-10 sm:py-16">
    ...
    <nav aria-label={sv ? "Hjälp och information" : "Help and information"} className="mb-8 flex flex-wrap gap-x-5 gap-y-2 border-y py-4 text-sm">
      {[["/documentation", sv ? "Kom igång" : "Getting started"], ["/federation", "Nolto & Mastodon"], ["/help", sv ? "Hjälp" : "Help"], ["/privacy", sv ? "Integritet" : "Privacy"]].map(([path, label]) =>
    ...
    <p className="mt-10 text-sm">{sv ? "Frågor?" : "Questions?"} <a className="underline" href={`mailto:${legalContact}`}>{legalContact}</a></p>
  </div>;
}
