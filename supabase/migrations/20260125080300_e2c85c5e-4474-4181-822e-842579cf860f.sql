-- Drop both versions and recreate with fixed parameter names
DROP FUNCTION IF EXISTS public.can_message_user(uuid, uuid);
DROP FUNCTION IF EXISTS public.can_message_user(uuid, uuid, uuid);

-- Recreate the function with properly named parameters to avoid column name conflicts
...
BEGIN
    ...
    -- Check for existing connection (fixed: use explicit table qualifiers)
    SELECT EXISTS (
        SELECT 1 FROM connections c
        WHERE c.status = 'connected'
        AND ((c.requester_id = p_sender_id AND c.recipient_id = p_recipient_id)
             OR (c.requester_id = p_recipient_id AND c.recipient_id = p_sender_id))
    ) INTO is_connected;
