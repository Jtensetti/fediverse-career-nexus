...
export interface SignupDeps {
  ...
  db?: () => Pick<SupabaseClient, "from" | "rpc" | "auth">;
  sendEmail?: typeof resendEmail;
}

/** Side-effect free until called; index.ts wires the real Deno.serve. Tests inject fakes. */
export const createSignupHandler = (deps: SignupDeps = {}) => async (req: Request): Promise<Response> => {
  const sendEmail = deps.sendEmail ?? resendEmail;
  if (req.method === "OPTIONS") return jsonResponse({});
  if (req.method !== "POST") return jsonResponse({ error: "Method not allowed" }, 405);
  const apiKey = deps.apiKey ? deps.apiKey() : Deno.env.get("RESEND_API_KEY");
