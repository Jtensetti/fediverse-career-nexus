...
/** Side-effect free until called; index.ts wires the real Deno.serve. Tests inject fakes. */
export const createConfirmHandler = (deps: ConfirmDeps = {}) => async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const db = deps.db ? deps.db() : serviceClient();
    ...
    if (new Date(data.expires_at) < now()) return jsonResponse({ error: "Token expired" }, 400);
    const { error: authError } = await db.auth.admin.updateUserById(data.user_id, { email_confirm: true });
    if (authError) throw authError;
    const { error: updateError } = await db.from("email_verification_tokens").update({ used_at: now().toISOString() }).eq("user_id", data.user_id);
