...
- Evidence: `supabase/tests/email-links.test.ts` (unlisted sibling falls back, listed sibling both directions, malformed/credential/path/query/port/wildcard entries never expand trust, preview/localhost exact-only, token round-trip to `/confirm-email`); `scripts/federation-gateway.test.mjs` asserts the split-mode GET redirect keeps `/confirm-email?token=` intact.
