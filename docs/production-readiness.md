...
This branch is a release candidate, not a certification that the live service is production ready. The code changes described below have passed the listed local checks; external integration and operational checks remain open. The existing Lovable Cloud backend is now identified and a read-only production preflight has run. Deployment, a staging migration rehearsal, a real Mastodon round trip and the operator's legal details remain release gates.
...
- The delivery ledger, one-time OAuth state and verified remote-identity tables do not exist yet. Neither production-readiness migration has been applied.
...
- The final read-only preflight also found zero messages (including zero legacy plaintext messages), zero company roles with missing users, and no live-session assurance policies yet. No credentials or user message content were selected.

Git's merge check found no conflicts with the two newer Lovable commits on `main`; they add preview auth storage and regenerate client types. The release branch now includes those main-branch changes. No merge into main or production mutation was performed.
...
| Messages | Encryption failure could fall back to plaintext; decrypt filters accepted unvalidated identifiers; reads could truncate | Server-owned encrypted sends, strict identifiers, participant RLS, bounded cursor pagination and explicit failure |
...
| Export | Partial queries and unrelated records could be presented as a complete archive | Explicit ownership/projections, complete paginated reads or failure, size limits, decrypted own messages and file metadata; one honest JSON format |
| Account deletion | Non-FK records and raw post content could survive; organisations could lose their owner | Deletion preflight, recent authentication, storage API removal, database cleanup, last-owner protection and permanent reservation of retired usernames |
...
- A restorable backup and staged restore, not just a SQL export. The available Lovable connector exposes querying and publishing but no backup/restore or secret-management operation. No production backup has been claimed or made in this session.
- A staged browser test of signup, confirmation, native recovery, MFA recovery, message send/read, company creation, exports, file deletion and the user journey on desktop/mobile. The available browser could not reach the local preview or `nolto.social`; no visual acceptance is claimed.
...
- Authenticated schedules for federation, maintenance, event reminders and digests, together with gateway rate limiting, egress controls, alert delivery and a restore/rollback owner. Maintenance APIs now require an administrator or service credential; event/digest workers accept only the service credential.
...
- A reviewed dry-run cleanup manifest and then actual deletion of the 50 marked demo users, preserving the real account. This remains outstanding.
...
Self-service deletion currently limits active federation recipients to 50; larger accounts need an operator-run background process. Remote account deletion attempts are best effort, without a durable post-deletion retry job. Storage deletion, remote delivery and Auth deletion are not one cross-service transaction; a failure can require a reviewed retry. Organisation files/sole ownership block deletion until transfer. Do not promise immediate universal erasure. Account exports have a 20 MiB / 50,000 rows-per-table ceiling and fail rather than quietly truncate; larger requests need assisted processing. They are not a single cross-table point-in-time database snapshot.

Existing users' contributions, ownership and files need a separate inventory before demo deletion. After verifying that no historical role rows refer to missing users, validate `company_roles_user_id_fkey`; it already prevents new invalid assignments. Inspect remaining application features and logs under staging traffic, including interaction notifications, local boosts/replies and large-account pagination. Static and database tests do not establish every UI integration.
...
- Two native Node tests for conservative demo selection; 20 Deno tests for canonical identity, parser rejection, actor ownership, real RSA signature verification/tampering, response-size boundaries, SSRF filters, PKCE vector, object normalization partial delivery failure/retry, encryption tampering/legacy reads, export pagination and malformed requests.
