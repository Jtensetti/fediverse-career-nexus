...
- Deployed the existing security/privacy handlers through Lovable. Retired the deployed demo seed and scheduled database-wipe endpoints. The scoped moderation endpoint is also deployed and rejects unauthenticated requests with HTTP 401. Frontend publication follows the final CI checks.
...
- Direct backend discovery returned HTTP 200 JRD for `jonatan_tensetti@nolto.social` and an ActivityPub Person with matching canonical actor/key-owner URLs. This does not test the custom-domain proxy or remote-server acceptance.
- Lovable regenerated the schema types after deployment; these now reflect the migrated database and pass TypeScript. Its preview-storage generator also overwrote the parent-window check; it was restored with a behavioral regression test for forged replies. Ignore Bun lockfiles and continue to use npm.
