...
Updated 22 September 2026. The privacy migrations, private Storage policies and authenticated maintenance scheduler are deployed. Backend and frontend releases must be verified separately; see [deployment requirements](production-readiness.md).
...
## Reviewing reported, deleted text

An administrator or moderator may explicitly open retained post/article text only for an existing report that was filed before deletion and remains pending or reviewed. An ordinary user cannot call the archive RPC. The server verifies the live session, MFA and role, checks the report/deletion binding, logs access and decrypts only the requested text. The interface does not expose full snapshots, images, account records or private messages. Comments do not yet have this review path.

The report cannot be backdated or pointed at another object through client writes. Access ends at the original deletion deadline even if the purge worker is delayed; opening a report never extends retention. The browser keeps the result only in component memory and clears it on closing/sign-out or expiry. Access audit rows are removed with the underlying deletion request. This does not provide a legal hold or promise moderators cannot copy text they legitimately view.
...
2. Provision a **new independent random** `RETENTION_ENCRYPTION_KEY` of at least 32 characters through server secret storage. Preserve the existing token/message encryption secret. Back up secrets separately from database/files with restricted access. The existing deployment has this independent secret; do not replace it without a tested re-encryption procedure.
