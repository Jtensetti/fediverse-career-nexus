...
This is an opt-in, experimental subset of the Mastodon client API. It is **not enabled on nolto.social**. Public-domain routes, migration `20260922184945` and matching handlers are deployed; the hosted API/OAuth handlers return the intended HTTP 503 gate. The seven tables, two invoker views, two ID triggers and cleanup RPC are verified. Full access requires `MASTODON_CLIENT_ENABLED=true`; the operator pilot described below is prepared in code but not activated. With neither enabled, account operations and discovery return 503 before database access. Do not enable full access until a real client has passed acceptance checks.
