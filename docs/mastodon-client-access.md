...
This opt-in, experimental subset of the Mastodon client API is **available to all Nolto accounts on nolto.social**, with `MASTODON_CLIENT_ENABLED=true`. Each user must explicitly authorize an app. Public-domain routes, migration `20260922184945` and matching handlers are deployed. The seven tables, two invoker views, two ID triggers and cleanup RPC are verified. Discovery and public feeds are public; user-specific operations still require a scoped user token. The operator pilot list is retained for rollback and does not restrict access while the full flag is true.

PR #78 deployed the pilot and repaired onboarding through canonical migration `20260923065928`, preserving actor identity, keys and access restrictions. Live Phanpy testing on 23 September confirmed OAuth login, correct identity, profile and home reload, public text posting, replies, likes and unlikes. Deletion through Nolto removed both test posts. Revoking app access blocked the home feed and left no active grants; reconnecting succeeded. These results cover the tested subset in Phanpy. Independent signed Follow/Accept exchanges and general compatibility with other Mastodon clients remain unverified.

After enabling full mode and redeploying the API/OAuth handlers, live checks confirmed discovery and anonymous public feeds, temporary app registration, client-credentials token issuance and app verification. The app-only token could read public data but received 401 for user identity and home. Revocation returned 200, then both public-feed and app-verification requests with that token returned 401. The temporary registration and its grants were removed. A fresh browser reload confirmed the expected empty Phanpy home after cleanup and one active Phanpy connection in Nolto's connected-app settings.
...
- Registration, consent, exchange and authenticated requests have database-backed rate limits. The existing privacy worker cleans expired codes/grants, revoked grants, idempotency keys and rate counters. App registrations remain so installed clients can reconnect. App revocation deletes outstanding codes and revokes all matching user grants in one transaction.
...
Pilot mode remains available for rollback and future restricted tests. Set `MASTODON_CLIENT_ENABLED=false` while retaining the validated operator list to restrict new account access and existing token use to those users. Remove the list as well to turn discovery and account operations off. Authenticated grant management/revocation remains available. Pilot actions are real posts and social interactions, not sandbox data. Full-mode tests must separately cover app-only tokens and anonymous public reads.

For a fresh deployment or a material authentication change:
...
5. Prefer an isolated staging deployment. If none is available, use the operator pilot on the verified deployment for user-authorized operations. Run against the chosen origin:

       node scripts/check-mastodon.mjs https://staging.example.com

6. With a consenting test user and the intended app, test registration → sign-in → explicit consent → callback → token → identity → feeds → public post/reply/favourite/unfavourite → revocation and reconnection. Keep the isolated session/MFA, replay, scope, unsupported-private-post and moderation tests passing. In full mode, verify temporary app-only credentials can verify the app and read public data but cannot access user identity/home or write; revoke the token and verify rejection. Invalid supplied tokens must never fall back to anonymous access. Clean up test posts and grants.
7. Record independent federation acceptance separately from native-client activation:

       node scripts/check-federation.mjs username@staging.example.com

   Exercise signed Follow/Accept/Note/reply/Like/Undo with a consenting real peer before claiming those cross-server exchanges are verified. This remains open work for the current release.
8. Publish activation status that matches the runtime and tested subset. HTTP 410 means the old stub; HTTP 503 saying access is not enabled means the gated function; SPA HTML at an API URL means incorrect domain routing. Preserve the pilot configuration as a reversible fallback.

On 23 September the connected Supabase control-plane account still rejects access to the hosted project. Deployment and runtime configuration succeeded through Lovable after credits were replenished; its read/query functions work. Cloudflare routing is deployed and verified; git synchronization alone does not establish a backend deployment.

Gateway changes were merged in PR #77 at `bb5c0d474a588af531a67bdf8d1c0e67c4935f80`. The live gateway passes all eight routing checks and public federation discovery for `jonatan_tensetti@nolto.social`. Regression coverage includes route coverage, split-domain redirects, signed inbox bytes through the proxy, website-origin consent and one-time browser returns. Hosted Phanpy acceptance is recorded above; gateway checks alone do not establish signed exchange with another server.
