...
Enabling managed Google/Apple OAuth also regenerates `src/integrations/lovable/index.ts`. It is subject to the same unreachable-module rule. The application imports the pinned managed OAuth SDK directly, restricts it to a top-level browser context and handles its result through the one active Supabase client. Its callback checks a separate one-use, 256-bit browser nonce with a 15-minute expiry before accepting returned tokens. URL auto-detection is disabled on that callback so an unsolicited fragment cannot establish a session before the check. Normal MFA verification remains required. Provider configuration can regenerate dependency entries; the checked-in npm lockfile must remain synchronized.

On the hosted Edge runtime, raw TLS to bsky.social terminates with `UnexpectedEof`. Only the fixed service domains `bsky.social`, `plc.directory` and hosts strictly beneath `.host.bsky.network` use the platform's native HTTPS fetch. These DNS zones are operated by the identity infrastructure, rather than selected or controlled by an arbitrary account holder. DNS answers are still checked for public addresses, TLS certificate verification remains on, redirects are rejected, response size is bounded and requests time out. Unknown PDS/issuer domains continue to require the IP-pinned TCP/TLS path; failure never falls back to unpinned fetch for them. This explicitly distinguishes trusted infrastructure from user-controlled discovery URLs.
