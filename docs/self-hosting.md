...
- Confirmation emails link to `SITE_URL`. The signup request's browser origin is used instead only when it exactly equals `SITE_URL` or an exact origin (scheme, host and port, no path) listed in the backend secret `EMAIL_LINK_ORIGINS` (comma-separated). A www/apex sibling of `SITE_URL` is **not** trusted automatically; list it explicitly if you serve both. Previews and `http://localhost:<port>` also need an exact entry.
- Fresh database: apply `supabase/migrations` to an empty project (`supabase start` / `supabase db push`). See `docs/fresh-install.md`.
